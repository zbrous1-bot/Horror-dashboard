...
Paste into horror_dashboard.py (replace all)
Commit → Redeploy

This version has all use_container_width replaced with width='stretch' and should finally work without errors. Let me know what happens!text2026-05-25 18:10:48.579 Uvicorn server started on 0.0.0.0:8501[18:13:01] ❗️ The service has encountered an error while checking the health of the Streamlit app: Get "http://localhost:8501/healthz": read tcp 127.0.0.1:45646->127.0.0.1:8501: read: connection reset by peer✅ Here's the cleanest, most robust version yet.
Copy everything below and replace your entire horror_dashboard.py file with it:
...
def load_horror_data():
    ...
    # Handle year column safely
    if 'release_year' in df.columns:
        df['year'] = df['release_year']
    elif 'year' not in df.columns:
        df['year'] = pd.to_datetime(df.get('release_date', pd.Series()), errors='coerce').dt.year
    
    # Safe features column
    df['features'] = df['overview'].fillna('') + ' found footage supernatural possession demon paranormal ghost haunted slasher psychological horror'
    df = df.reset_index(drop=True)
    return df
...
with tab2:
    ...
    if len(st.session_state.watched) == 0:
        ...
        st.info("Recommendations coming soon... (basic version for now)")
