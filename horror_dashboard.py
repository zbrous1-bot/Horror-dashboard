...
import os\
\
st.set_page_config(page_title="Horror Movie Tracker", page_icon="\uc0\u55357 \u56443 ", layout="centered")\
st.title("\uc0\u55357 \u56443  Horror Dashboard")\
\
@st.cache_data\
...
def smart_match(title, year=None):\
    ...
    return None\
\
st.sidebar.header("\uc0\u55357 \u56549  Import from Letterboxd")\
...
\
st.sidebar.subheader("\uc0\u10133  Add Manually")\
...
\
tab1, tab2, tab3 = st.tabs(["\uc0\u55357 \u56523  Watched", "\u55356 \u57263  Recommendations", "\u55357 \u56589  Search"])\
\
with tab1:\
    ...
    if len(st.session_state.watched) > 0:\
        ...
        if st.session_state.watched['rating'].notna().any():\
            col2.metric("Avg Rating", f"\{st.session_state.watched['rating'].mean():.1f\} \uc0\u11088 ")\
...
\
with tab2:\
    st.header("\uc0\u55356 \u57263  Recommendations For You")\
    if len(st.session_state.watched) == 0:\
        ...
        if len(watched_idx) > 0:\
            ...
            for _, row in recs.iterrows():\
                with st.container():\
                    ...
                    st.caption(f"Director: \{row.get('director', 'N/A')\} \'95 TMDB: \{row['vote_average']:.1f\}")\
                    ...
                    with col2:\
                        if st.button("\uc0\u9989  Mark as Watched", key=f"w_\{int(row['id'])\}", use_container_width=True):\
                            new_entry = pd.DataFrame([\{'title': row['title'], 'year': row['year'], 'rating': rating, 'matched_id': int(row['id'])\}])\
                            st.session_state.watched = pd.concat([st.session_state.watched, new_entry]).drop_duplicates()\
                            save_watched_list(st.session_state.watched)\
                            st.toast(f"Added with \{rating\} stars!", icon="\uc0\u11088 ")\
                            st.rerun()\
                    st.link_button("\uc0\u55357 \u56599  TMDB", f"https://www.themoviedb.org/movie/\{int(row['id'])\}", use_container_width=True)\
                    st.divider()\
\
with tab3:\
    st.header("\uc0\u55357 \u56589  Check If You've Seen It")\
    ...
    if q:\
        ...
        if best_row is not None:\
            ...
            if seen:\
                st.success("\uc0\u9989  You've seen this!")\
            else:\
                st.warning("\uc0\u10060  Not in your list yet")\
...
\
st.sidebar.caption("Year-aware matching \'95 Auto-saves \uc0\u11088 ")}
